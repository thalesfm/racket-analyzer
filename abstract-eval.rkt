...
(provide make-abstract-evaluator)
...
; (define (make-abstract-evaluator language) (error))

(define (make-abstract-evaluator domain α)
  (define T (domain:T domain))
  (define ⊥ (domain:⊥ domain))
  (define T? (curry domain:T? domain))
  (define ⊥? (curry domain:⊥? domain))

  (define (<=? v1 v2)
    (cond
     [(and (closure? v1) (closure? v2)) (closure<=? domain v1 v2)]
     [(or  (closure? v1) (closure? v2)) #f]
     [else (domain:<=? domain v1 v2)]))

  (define (lub v1 v2)
    (cond
     [(and (closure? v1) (closure? v2)) (closure-lub domain v1 v2)]
  ...
  (define (abstract-eval-syntax stx env [trace (hasheq)])
    (syntax-parse stx
      ...
      [lit:literal (α (syntax->datum #'lit.datum))]
      [lam:lambda-expr
       (define captured-env
