...
(check-eq? (partial-eval '(read)) Top)
(check-eq? (partial-eval '(+ (read) 10)) Top)
...
(check-true (type<=? (partial-eval '(if (read) 10 11)) Number))

(check-eq? (partial-eval '(error)) Bot)
(check-eq? (partial-eval '(let ([x (error)]) 'unused)) Bot)
(check-eq? (partial-eval '(letrec ([x (error)]) 'unused)) Bot)
(check-eq? (partial-eval '((error) 'unused)) Bot)
(check-eq? (partial-eval '(+ (error) 10)) Bot)
(check-eq? (partial-eval '(+ 10 (error))) Bot)

(check-eq? (partial-eval 'x) Bot)
(check-eq? (partial-eval '(let ([x 10]) y)) Bot)
